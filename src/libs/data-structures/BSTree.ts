...
import {IDistanceFunction} from './util';
...
export default class BSTree<T> {
  ...
  /**
   * Finds the tree element which is closest to an element
   *
   * @param element - the element to compare tree elements to
   * @param distanceFunc - a function caclualting the distance between 2 elements. e.g abs(elt1-elt2)
   * @returns - closest element to element in the tree or null if the tree has no elements
   */
    public findClosestElement(element:T, distanceFunc:IDistanceFunction<T>):T {
      if(this.isEmpty()) return null;
      let res:T = this.root.element;
      let node = this.root;

      console.log(`Closest: ${res.toString()}`);

      while (node !== null) {
        const distance = distanceFunc(element,node.element);
        if (this.compare(distance,res) < 0) {
          res = node.element;
          console.log(`Closest: ${res.toString()}`);
        }
